...
Ноды управляются по обычному SSH (агент на них не ставится). Поддерживаются четыре протокола сразу: **AmneziaWG**, **AmneziaWG 3.0**, **OpenVPN поверх Cloak** и **XRay / REALITY**. Клиенты выдаются в формате AmneziaVPN `vpn://` (со сканируемым анимированным QR), а для WireGuard — ещё и обычным `.conf`.
...
- Разворачивание AmneziaWG / AmneziaWG 3.0 / XRay / OpenVPN-over-Cloak на чистый сервер (сборка на самой ноде, с сохранением клиентов)
- **AmneziaWG 3.0** — отдельный протокол рядом с 2.0 и legacy: свой контейнер, порт и подсеть, поэтому обе версии живут на одной ноде
...
Установка 3.0 — **аддитивная**: поднимается отдельный контейнер (`amnezia-awg3`)
со своим портом, подсетью и каталогом конфига. Работающие 2.0/legacy при этом не
пересоздаются и не перенастраиваются — проверено на боевой ноде: после установки
3.0 у контейнера 2.0 остались тот же container ID, тот же хеш конфига, те же
клиенты и живые рукопожатия.

Перед установкой панель снимает **бэкап всех AmneziaWG-контейнеров ноды**
(`~/.acontrol/snapshots/…`), а порт проверяется заранее: если он занят другим
контейнером, развёртывание не начнётся (иначе скрипт снёс бы занявшего порт).

Полный откат 3.0 к состоянию «как было» — на ноде:
...
sudo ufw delete allow <порт 3.0>/udp   # если используется ufw
```

Клиенты 2.0 это не затрагивает. Если нужно вернуть **конфиг** (не удалить
протокол) — в панели на вкладке протокола есть «Откатить» со списком снимков.
