...
async def test_create_stores_config_and_has_config_flag(
    ...
    monkeypatch.setattr(awg, "read_state", lambda conn, host: _wrap(_state([existing])))
    ...
    monkeypatch.setattr(
        awg, "read_state", lambda conn, host: _wrap(_state([existing, new_client]))
    )
...
async def test_get_stored_config(
    ...
    sid = await _make_server(client, auth_headers)
    monkeypatch.setattr(awg_api, "_connect", lambda server: _DummyConn())
    monkeypatch.setattr(awg, "read_state", lambda conn, host: _wrap(_state([])))
...
async def test_note_set_and_shown(
    ...
    monkeypatch.setattr(awg, "read_state", lambda conn, host: _wrap(_state([peer])))
...
async def test_create_with_note(
    ...
    monkeypatch.setattr(awg, "read_state", lambda conn, host: _wrap(_state([nc])))
    r = await client.get(f"/api/servers/{sid}/awg", headers=auth_headers)
